...
BOOL CPeUtil::RepairRelocLocal(DWORD imageBase)
{
...
{
	//修复壳代码的重定位表指向的地址
	RepairRelocLocal(imageBase);

	
	//PIMAGE_DOS_HEADER pDosHeaderDll = (PIMAGE_DOS_HEADER)imageBase;
	//PIMAGE_NT_HEADERS pNtHeaderDll = (PIMAGE_NT_HEADERS)(pDosHeaderDll->e_lfanew + imageBase);
	//PIMAGE_OPTIONAL_HEADER pOptionalHeaderDll = (PIMAGE_OPTIONAL_HEADER)(&pNtHeaderDll->OptionalHeader);
	//IMAGE_DATA_DIRECTORY relocDataDirectoryDll = (IMAGE_DATA_DIRECTORY)(pOptionalHeaderDll->DataDirectory[5]);
	//PIMAGE_BASE_RELOCATION pBaseRelocationDll = (PIMAGE_BASE_RELOCATION)(relocDataDirectoryDll.VirtualAddress + imageBase);
	//PIMAGE_SECTION_HEADER pSectionHeaderDll = IMAGE_FIRST_SECTION(pNtHeaderDll);

	////修改重定位表的virtualAddress值
	//while (pBaseRelocationDll->SizeOfBlock >= 8)
	//{

	//	//pBaseRelocationDll->VirtualAddress - pSectionHeaderDll->VirtualAddress = newVA - GetLastSection()->VirtualAddress;
	//	//DWORD newVA = pBaseRelocationDll->VirtualAddress - pSectionHeaderDll->VirtualAddress + GetLastSection()->VirtualAddress;
	//	pBaseRelocationDll->VirtualAddress= pBaseRelocationDll->VirtualAddress - pSectionHeaderDll->VirtualAddress + GetLastSection()->VirtualAddress;

	//	pBaseRelocationDll = (PIMAGE_BASE_RELOCATION)((DWORD)pBaseRelocationDll + pBaseRelocationDll->SizeOfBlock);
	//}


	////修改dataDirctory[5].virtualAdress的值，& 修改dataDirctory[5].size
	////dll中reloc rva
	////relocDataDirectoryDll.VirtualAddress - pSectionHeaderDll->VirtualAddress = newVA - GetLastSection()->VirtualAddress;
	//DWORD newVA = relocDataDirectoryDll.VirtualAddress - pSectionHeaderDll->VirtualAddress + GetLastSection()->VirtualAddress;
	//IMAGE_DATA_DIRECTORY relocDataDirectory = pOptionalHeaderDll->DataDirectory[5];
	//relocDataDirectory.VirtualAddress = newVA;
	//relocDataDirectory.Size = relocDataDirectoryDll.Size;
